...
#' Convert between c3d points data formats
#'
#' Convert between different representation of point data in c3d files.
#'
#' @param x A data.frame of class \code{c3d_data} as generated by
#'   \code{\link{c3d_data}}.
...
#'
#' @export
c3d_convert <- function(x, format) {
  if (!inherits(x, "c3d_data")) stop("'x' needs to be a data frame of class 'c3d_data' as generated by 'cd3_data()'.")
  ...
  if (inherits(x, "c3d_data_wide")) {
    if (format == "wide") {
      ...
      c3d_long_to_longest(c3d_wide_to_long(x))
    } else {
      stop("'format' argument must be either 'wide', 'long', or 'longest'")
    }
  # input: long
  } else if (inherits(x, "c3d_data_long")) {
    if (format == "wide") {
      c3d_long_to_wide(x)
    } else if (format == "long") {
      ...
      c3d_long_to_longest(x)
    } else {
      stop("'format' argument must be either 'wide', 'long', or 'longest'")
    }
  } else if (inherits(x, "c3d_data_longest")) {
    if (format == "wide") {
      c3d_long_to_wide(c3d_longest_to_long(x))
    } else if (format == "long") {
      c3d_longest_to_long
    } else if (format == "longest") {
...
c3d_long_to_wide <- function(x) {
  if (!inherits(x, "c3d_data_long")) stop("'x' needs to be a data frame of class 'c3d' in the long format.")
...
c3d_longest_to_long <- function(x) {
  if (!inherits(x, "c3d_data_longest")) stop("'x' needs to be a data frame of class 'c3d' in 'longest' format.")
